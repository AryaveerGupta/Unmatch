...
pics = pd.read_csv("noon_pics.csv")
array_pics = pics["urls "]
array_pics = array_pics.to_numpy() 
...
flag = int(input("for url input give 1 else 0."))
if flag!=1 :  
 main_str =  input("path of file in system.") 
 main_img = Image.open(main_str) 

else : 
 main_str =  input("Give the input url.")
 urllib.request.urlretrieve(main_str,"pic1.jpg")
 main_img = Image.open("pic1.jpg")
...
def Image_hash(main_img , array_pics) : 
   ...
   output = []
   for i in range(array_pics.shape[0]) :   
